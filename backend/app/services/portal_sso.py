"""Decode the existing ai4edu portal's signed Flask session cookie.

The school deployment reuses the portal session after jAccount has completed
authentication.  Compass only reads the stable jAccount identifier and
display name; it never receives or stores the portal OAuth access token.
"""
...
def decode_portal_identity(cookie_value: str | None) -> dict[str, str] | None:
