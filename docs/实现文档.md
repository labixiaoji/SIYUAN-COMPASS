...
项目只保留 `docker-compose.yml`，且它仅用于本地开发。它定义三个服务：

- `postgres`：PostgreSQL 16，数据持久化到 `postgres-dev-data` volume。
- `backend`：FastAPI，挂载本地源码并使用 `uvicorn --reload`。
- `frontend`：Node.js + Vite 开发服务器，挂载本地前端源码。
