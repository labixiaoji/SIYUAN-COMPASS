...
- 问卷草稿和报告预填按 `user.id` 使用不同的 `localStorage` key，记录明确的过期时间；问卷草稿优先保存到云端，云端默认保留 30 天，本地草稿最长保存 7 天作为网络异常兜底。
...
- 云端草稿接口：`GET/PUT/DELETE /api/assessment-draft`；接口按登录账号读写，不接受客户端传入 `userId`。`PUT` 使用 `version` 做乐观并发控制，版本不一致返回 `409`。
...
- 启动时执行：
  ...
  - 清理超过保存期限的终态任务、云端问卷草稿、管理员审计日志、历史画像原始输出和历史问卷禁止保存字段。
  - 清零已经跨过自然日的账号级生成配额计数。
...
| `ASSESSMENT_DRAFT_RETENTION_DAYS` | 云端问卷草稿保留天数，默认 `30` 天 |
...
| `assessment_drafts` | 当前账号的一份云端问卷草稿，含部分答案、当前步骤、版本号和过期时间 |
...
erDiagram
  ...
  users ||--o| assessment_drafts : edits
...
- `assessment_drafts` 按 `user_id` 唯一约束，每个账号最多一份草稿；答案写入 JSONB，接口只根据当前登录账号查询，不接受客户端传入的用户 ID。草稿默认保留 30 天，提交创建报告任务后删除，过期草稿由每日维护任务清理。保存使用版本号和行锁，避免多设备旧内容覆盖新内容。
- `claim_token` 与 `lease_expires_at` 用于任务恢复和多 worker 排他领取；按 `generation_job_id` 建立的报告版本唯一索引避免恢复时重复写版本。

数据删除均在 PostgreSQL 事务中完成：单份报告删除会清理关联且不再被其他报告引用的问卷/画像；全部业务数据删除会利用外键级联清理当前学生的问卷、画像、报告、版本、反馈和云端草稿，并显式删除生成任务。登录账号保留。
...
页面包含本地字段级校验、按账号隔离的云端草稿自动保存/恢复（本地 7 天草稿作为网络异常兜底）、防重复提交、可取消的生成进度轮询、失败重试和返回修改问卷。云端草稿保存当前步骤和版本号，多设备发生冲突时提示刷新恢复最新版本。提交创建报告任务后清除当前用户草稿并跳转到报告详情页。
...
| `GET` | `/api/assessment-draft` | 读取当前账号未过期云端草稿 |
| `PUT` | `/api/assessment-draft` | 保存当前账号草稿；携带当前步骤和版本号 |
| `DELETE` | `/api/assessment-draft` | 删除当前账号云端草稿 |
...
- 草稿按用户隔离：云端默认 30 天过期并由每日维护清理，本地草稿 7 天过期并在退出时清理。
...
- 问卷草稿按用户隔离；云端默认 30 天后过期，本地草稿 7 天后过期，退出时清理当前用户本地草稿。
